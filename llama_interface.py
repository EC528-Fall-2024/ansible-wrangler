...
# #     playbook_contents = []  # Flattened for FAISS
# #     formatted_playbooks = []  # Original formatting for saving
...
# #                     formatted_playbooks.append(content)  # Preserve original
...
# #             index_cpu = faiss.index_gpu_to_cpu(index_gpu)  # Transfer back to CPU for saving
# #         except Exception as e:
...
# #     # Save original formatted playbooks
# #     try:
# #         with open(DOCUMENTS_PATH, 'w', encoding='utf-8') as f:
# #             for doc in formatted_playbooks:
# #                 f.write(doc + "\n---\n")  # Separate playbooks with "---"
# #         print(f"Playbook contents saved to '{DOCUMENTS_PATH}'.")
# #     except Exception as e:
...
#     # Read playbooks
#     playbook_contents = []  # Flattened playbooks for embeddings
#     formatted_playbooks = []  # Original playbooks for saving
...
#                     formatted_playbooks.append(content.strip())  # Keep original formatting
...
#     """
#     global index_gpu, index_cpu, documents  # Ensure global variables are used
...
#         index_gpu = None  # Ensure GPU index is None if not used
...
#     """
#     global index_gpu, index_cpu  # Ensure global variables are used
...
#         if index is None:
#             raise ValueError("FAISS index is not loaded. Ensure the retrieval system is initialized.")
...
#     # Load and split the playbooks from DOCUMENTS_PATH
#     try:
#         with open(DOCUMENTS_PATH, 'r', encoding='utf-8') as f:
...
#             retrieved.append(all_playbooks[idx].strip())  # Append formatted playbooks
#         else:
