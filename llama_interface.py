...
def create_faiss_index():
    """
    Create a FAISS-GPU index from existing Ansible playbooks.
    """
    ...
    try:
        index_cpu = faiss.IndexFlatL2(dimension)  # Simple flat index
        print("FAISS CPU index created successfully.")
    except Exception as e:
        print("Error creating FAISS CPU index:", e)
        return
    ...
    try:
        ...
        print("Embeddings added to GPU index successfully.")
    except Exception as e:
        print("Error adding embeddings to GPU index:", e)
        return
    ...
    try:
        ...
        print("Error transferring FAISS index back to CPU:", e)
        return
...
def load_retrieval_system():
    """
    Load the FAISS-GPU index and playbook documents.
    Returns:
        index_gpu (faiss.Index): FAISS index on GPU.
    ...
    print("Loading FAISS index from CPU...")
    try:
        index_cpu = faiss.read_index(FAISS_INDEX_PATH)
        print("FAISS index loaded from CPU successfully.")
    except Exception as e:
    ...
    return index_gpu, documents

def retrieve_playbooks(query, top_k=TOP_K):
    """
...
def generate_ansible_playbook(task_description):
    ...
    retrieved_playbooks = retrieve_playbooks(task_description)
...
def evaluate_playbooks_with_llama(existing_playbooks, description):
    ...
    retrieved_playbooks = retrieve_playbooks(description)
...
def main():
    ...
    if command == 'index':
        ...
        playbook = generate_ansible_playbook(task_description)
        ...
        if not existing_playbooks:
            ...
            sys.exit(1)
        matching_playbook = evaluate_playbooks_with_llama(existing_playbooks, description)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2 and sys.argv[1].lower() == 'index':
        ...
        if os.path.exists(FAISS_INDEX_PATH) and os.path.exists(DOCUMENTS_PATH):
            ...
            index_gpu, documents = load_retrieval_system()
        else:
